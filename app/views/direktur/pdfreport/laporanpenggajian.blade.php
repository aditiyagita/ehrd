...
<center>
	...
	<table width=100% class="laporan">
		...
		<tr>
			...
			<td width=10% align="center">Gaji Pokok</td>
			<td width=10% align="center">Uang Harian</td>
			<td width=10% align="center">Uang Lembur</td>
		...
		<tr>
			...
			<td>{{$penggajian->karyawan->department->department}}</td>
			<td align="center">{{number_format(($penggajian->karyawan->gaji), 0, ',', '.')}}</td>
			<td align="center">{{number_format((($penggajian->hadir-$penggajian->terlambat)*$penggajian->karyawan->tunjangan_harian), 0, ',', '.')}}</td>
			<td align="center">{{number_format(($penggajian->jamlembur*20000), 0, ',', '.')}}</td>
			<td align="center">{{ number_format(((($penggajian->hadir-$penggajian->terlambat)*$penggajian->karyawan->tunjangan_harian)+$penggajian->karyawan->gaji+($penggajian->jamlembur*20000)), 0, ',', '.') }}</td>
		</tr>
		<?php $i++; ?>
		@endforeach
		</tbody>
